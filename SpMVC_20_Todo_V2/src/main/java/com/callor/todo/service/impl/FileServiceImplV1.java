...
public class FileServiceImplV1 extends FileServiceABS{
	...
	public Map<String,String> fileUp(MultipartFile file) {
		...
		// 매개변수로 받은 파일정보가 없으면 더이상 진행하지 말기
		if(file.isEmpty()) {
		...
		// UUID + "-" + 원래이름
		String saveFileName
		= String.format("%s-%s", strUUID, originalFileName);
		
		// 저장할 폴더와 파일이름을 매개변수로 전달하여
		// 파일을 저장하기 위하여 File 개체 생성
		...
		// 오리지널이름
		retFileName.put(QualifierConfig.FILE_SERVICE.ORIGINAL, 
				originalFileName);
		...
		// 오리지널이름 + 저장용 파일 이름을 map으로 return
		return retFileName;
	}
