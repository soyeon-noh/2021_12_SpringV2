...
public class HomeController {
	 ...
	 * 		방지할 수 있다.
	 * 
	 * 		@RequestParam("to_image", required =false) MultipartFile to_image 와 같이
	 * 		required = false 를 지정하고 defaultValue 를 지정하여 400 오류를 방지할 수 있다.
	 * 
	 * 2. Map<String, Object> 을 사용하여 데이터 받기 (요즘엔 권장하지 않음)
	 * 		form 에서 POST 로 전송된 데이터는 Map을 사용하여 쉽게 수신할 수 있다.
	 * 
	 * 		@RequestBody 를 사용해서 변수를 설정한다
	 * 		@RequestBody 는 한개의 매개변수만을 사용할 수 있다.
	 * 		(RequestBody는 문제를 발생시킬 수 있기때문에 비추) 
	 * 
	 ...
	 * 		반드시 Getter, Setter, 필드생성자(AllarguConstructor)가 있어야 한다.
	 */
	@ResponseBody
	@RequestMapping(value="/upload", method=RequestMethod.POST)
	public Map<String, String> upload(
			 // @ModelAttrivute를 적지않아도 되지만 명확하게 하기 위함
